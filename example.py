...
# setup logging
logger = Logger(filename='example.log')
...
np.random.seed(1234)
GBRBM.srng = RandomStreams(np.random.randint(2**30))
...
X_train = dataset[:-1000, :]    # all but last 1000 samples for training
X_test  = dataset[-1000:, :]    # last 1000 samples for testing
X_trainsub = dataset[:1000, :]  # subset of training data with same number of samples as testset
...
inference_model = Sequential([h_given_x])
